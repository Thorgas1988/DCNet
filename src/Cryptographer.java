	...
	private class Handler extends NetworkThread
	{
		...
		Runnable continuesReader = new Runnable()
		{
			...
			public void run()
			{
				while (true)
				{
					System.out.println(getName() + ": Give input, allowed length:" + (keyStorage.keyLength) + ":");
				...
				throws IOException
		{
			...
			if (DCNet.mode == DCNet.Mode.MODE_TIMED_DC_NET && isCommandLine)
			{
			...
			if (DCNet.mode == DCNet.Mode.MODE_RANDOM_TIMED_CONFLICT_DC_NET)
			{
				startTimerRandomly();
			}
		}
		...
		public void runSpecialized() throws IOException
		{
			...
			while (true)
			{
				...
				if (isCommandLine
						&& (DCNet.mode == DCNet.Mode.MODE_BASIC_DC_NET || DCNet.mode == DCNet.Mode.MODE_LARGER_DC_NET || DCNet.mode == DCNet.Mode.MODE_DC_NET_FOR_BENCHMARKING))
				{
					Scanner in = new Scanner(System.in);
